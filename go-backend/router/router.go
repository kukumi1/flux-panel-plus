...
func Setup(r *gin.Engine) {
	...
	r.POST("/flow/audit", handler.AuditUpload)
	...
	r.GET("/s/:secret/init-lite", handler.CamoLiteInstallScript)
	r.GET("/s/:secret/audit-watchdog", handler.CamoAuditWatchdogScript)
	...
	auth.Use(middleware.JWT())
	{
		...
		auth.POST("/user/telegram/status", handler.TelegramStatus)
		auth.POST("/user/telegram/bind-code", handler.TelegramBindCode)
		auth.POST("/user/telegram/unbind", handler.TelegramUnbind)
		...
		auth.POST("/node/install-lite", middleware.Admin(), handler.NodeInstallLite)
		...
		// Route
		auth.POST("/route/create", middleware.Admin(), handler.RouteCreate)
		auth.POST("/route/list", middleware.Admin(), handler.RouteList)
		auth.POST("/route/update", middleware.Admin(), handler.RouteUpdate)
		auth.POST("/route/delete", middleware.Admin(), handler.RouteDelete)
		...
		// Audit
		auth.POST("/audit/list", middleware.Admin(), handler.AuditList)

		// HA Group
		auth.POST("/group/create", middleware.Admin(), handler.GroupCreate)
		auth.POST("/group/list", middleware.Admin(), handler.GroupList)
		auth.POST("/group/update", middleware.Admin(), handler.GroupUpdate)
		auth.POST("/group/delete", middleware.Admin(), handler.GroupDelete)
		auth.POST("/group/update-order", middleware.Admin(), handler.GroupUpdateOrder)

		// DDNS
		auth.POST("/ddns/provider/types", middleware.Admin(), handler.DdnsProviderTypes)
		auth.POST("/ddns/provider/create", middleware.Admin(), handler.DdnsProviderCreate)
		auth.POST("/ddns/provider/list", middleware.Admin(), handler.DdnsProviderList)
		auth.POST("/ddns/provider/update", middleware.Admin(), handler.DdnsProviderUpdate)
		auth.POST("/ddns/provider/delete", middleware.Admin(), handler.DdnsProviderDelete)
		auth.POST("/ddns/domain/create", middleware.Admin(), handler.DdnsDomainCreate)
		auth.POST("/ddns/domain/list", middleware.Admin(), handler.DdnsDomainList)
		auth.POST("/ddns/domain/update", middleware.Admin(), handler.DdnsDomainUpdate)
		auth.POST("/ddns/domain/delete", middleware.Admin(), handler.DdnsDomainDelete)
		auth.POST("/ddns/domain/resolve", middleware.Admin(), handler.DdnsDomainResolve)

		// System Log
		auth.POST("/system-log/list", middleware.Admin(), handler.SystemLogList)
