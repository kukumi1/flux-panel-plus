...
func CreateForward(d dto.ForwardDto, userId int64, roleId int, userName string) dto.R {
	...
	if d.RouteId != 0 {
		return createRouteForward(d, userId, roleId, userName)
	}
	if d.TunnelId == 0 {
		return dto.Err("tunnelId or routeId is required")
	}
...
func GetAllForwards(userId int64, roleId int) dto.R {
	...
	query := `SELECT f.*, COALESCE(t.name, r.name) as tunnel_name, r.name as route_name, COALESCE(NULLIF(f.listen_ip,''), t.in_ip, rh.node_ip) as in_ip, COALESCE(t.type, 3) as tunnel_type, t.out_ip
		FROM forward f LEFT JOIN tunnel t ON f.tunnel_id = t.id LEFT JOIN route r ON f.route_id = r.id LEFT JOIN route_hop rh ON rh.route_id = r.id AND rh.hop_order = 0`
...
func UpdateForward(d dto.ForwardUpdateDto, userId int64, roleId int) dto.R {
	...
	if existForward.RouteId != 0 || d.RouteId != 0 {
		return updateRouteForward(d, existForward, userId, roleId)
	}
...
func DeleteForward(id int64, userId int64, roleId int) dto.R {
	...
	if forward.RouteId != 0 {
		return deleteRouteForward(forward)
	}
...
func PauseForward(id int64, userId int64, roleId int) dto.R {
	...
	if forward.RouteId != 0 {
		return pauseRouteForward(forward)
	}
...
func ResumeForward(id int64, userId int64, roleId int) dto.R {
	...
	if forward.RouteId != 0 {
		return resumeRouteForward(forward)
	}
...
func ForceDeleteForward(id int64) dto.R {
	...
	func() {
		defer func() { recover() }()
		if forward.RouteId != 0 {
			_ = cleanupRouteForwardServices(&forward)
			return
		}
		var tunnel model.Tunnel
		if err := DB.First(&tunnel, forward.TunnelId).Error; err != nil {
	...
	DB.Where("forward_id = ?", id).Delete(&model.ForwardRoutePort{})
	if err := DB.Delete(&model.Forward{}, id).Error; err != nil {
...
func DiagnoseForward(id int64, userId int64, roleId int) dto.R {
	...
	if forward.RouteId != 0 {
		return diagnoseRouteForward(forward)
	}
...
func getAllUsedPortsOnNode(nodeId int64, excludeForwardId *int64) map[int]bool {
	...
	// 3. Collect route entry ports where this node is the first route hop.
	txEntry := DB.Table("forward").Select("forward.in_port").Joins("JOIN route_hop ON route_hop.route_id = forward.route_id AND route_hop.hop_order = 0").Where("route_hop.node_id = ?", nodeId)
	if excludeForwardId != nil {
		txEntry = txEntry.Where("forward.id != ?", *excludeForwardId)
	}
	var routeEntryForwards []model.Forward
	txEntry.Scan(&routeEntryForwards)
	for _, f := range routeEntryForwards {
		if f.InPort != 0 {
			usedPorts[f.InPort] = true
		}
	}

	// 4. Collect route relay ports allocated on this node.
	txRelay := DB.Model(&model.ForwardRoutePort{}).Where("node_id = ?", nodeId)
	if excludeForwardId != nil {
		txRelay = txRelay.Where("forward_id != ?", *excludeForwardId)
	}
	var routePorts []model.ForwardRoutePort
	txRelay.Select("relay_port").Find(&routePorts)
	for _, p := range routePorts {
		if p.RelayPort != 0 {
			usedPorts[p.RelayPort] = true
		}
	}
...
func createRouteForward(d dto.ForwardDto, userId int64, roleId int, userName string) dto.R {
	if roleId != adminRoleId {
		return dto.Err("route forwards are admin-only in this version")
	}
	route, hops, errMsg := LoadRouteWithHops(d.RouteId)
	if errMsg != "" {
		return dto.Err(errMsg)
	}
	if route.Status != tunnelStatusActive {
		return dto.Err("route is disabled")
	}

	inPort, ports, portErr := allocateRouteForwardPorts(route, hops, d.InPort, nil)
	if portErr != "" {
		return dto.Err(portErr)
	}
	...
	forward := model.Forward{
		...
		InterfaceName: d.InterfaceName,
		InPort:        inPort,
		OutPort:       ports[len(ports)-1].RelayPort,
		Status:        forwardStatusActive,
		Inx:           maxInx + 1,
		CreatedTime:   now,
		UpdatedTime:   now,
	...
	if err := tx.Create(&forward).Error; err != nil {
		tx.Rollback()
		return dto.Err("failed to create route forward")
	}
	for i := range ports {
		ports[i].ForwardId = forward.ID
		ports[i].RouteId = route.ID
		ports[i].CreatedTime = now
		ports[i].UpdatedTime = now
		if err := tx.Create(&ports[i]).Error; err != nil {
			tx.Rollback()
			return dto.Err("failed to create route forward ports")
		}
	}
	...
	serviceName := buildServiceName(forward.ID, forward.UserId, nil)
	if gostErr := createRouteGostServices(&forward, route, hops, ports, nil, serviceName); gostErr != "" {
		DB.Model(&model.Forward{}).Where("id = ?", forward.ID).Update("status", forwardStatusError)
		return dto.Err(gostErr)
	}
	return dto.OkMsg()
}

func updateRouteForward(d dto.ForwardUpdateDto, existing *model.Forward, userId int64, roleId int) dto.R {
	if roleId != adminRoleId {
		return dto.Err("route forwards are admin-only in this version")
	}
	routeId := d.RouteId
	...
	if routeId == 0 {
		return dto.Err("routeId is required")
	}
	route, hops, errMsg := LoadRouteWithHops(routeId)
	if errMsg != "" {
		return dto.Err(errMsg)
	}
	if route.Status != tunnelStatusActive {
	...
	specifiedInPort := d.InPort
	if specifiedInPort == nil {
		specifiedInPort = &existing.InPort
	}
	inPort, ports, portErr := allocateRouteForwardPorts(route, hops, specifiedInPort, &existing.ID)
	if portErr != "" {
		return dto.Err(portErr)
	}

	if cleanupErr := cleanupRouteForwardServices(existing); cleanupErr != "" {
		return dto.Err(cleanupErr)
	}
	...
	tx := DB.Begin()
	if err := tx.Where("forward_id = ?", existing.ID).Delete(&model.ForwardRoutePort{}).Error; err != nil {
		tx.Rollback()
		return dto.Err("failed to update route forward ports")
	}
	if err := tx.Model(&model.Forward{}).Where("id = ?", existing.ID).Updates(map[string]interface{}{
		"name":           updated.Name,
		"tunnel_id":      0,
		"route_id":       updated.RouteId,
		"remote_addr":    updated.RemoteAddr,
		"strategy":       updated.Strategy,
		"listen_ip":      updated.ListenIp,
		"interface_name": updated.InterfaceName,
		"in_port":        updated.InPort,
		"out_port":       updated.OutPort,
		"status":         updated.Status,
		"updated_time":   updated.UpdatedTime,
	}).Error; err != nil {
		tx.Rollback()
		return dto.Err("failed to update route forward")
	}
	for i := range ports {
		ports[i].ForwardId = existing.ID
		ports[i].RouteId = route.ID
		ports[i].CreatedTime = now
		ports[i].UpdatedTime = now
		if err := tx.Create(&ports[i]).Error; err != nil {
	...
	serviceName := buildServiceName(updated.ID, updated.UserId, nil)
	if gostErr := createRouteGostServices(&updated, route, hops, ports, nil, serviceName); gostErr != "" {
		DB.Model(&model.Forward{}).Where("id = ?", updated.ID).Update("status", forwardStatusError)
		return dto.Err(gostErr)
	}
	return dto.Ok("route forward updated")
}

func deleteRouteForward(forward *model.Forward) dto.R {
	if cleanupErr := cleanupRouteForwardServices(forward); cleanupErr != "" {
		return dto.Err(cleanupErr)
	}
	tx := DB.Begin()
	if err := tx.Where("forward_id = ?", forward.ID).Delete(&model.ForwardRoutePort{}).Error; err != nil {
		tx.Rollback()
		return dto.Err("failed to delete route forward ports")
	}
	if err := tx.Delete(&model.Forward{}, forward.ID).Error; err != nil {
		tx.Rollback()
		return dto.Err("failed to delete route forward")
	}
	if err := tx.Commit().Error; err != nil {
		return dto.Err("failed to delete route forward")
	}
	return dto.Ok("route forward deleted")
}

func pauseRouteForward(forward *model.Forward) dto.R {
	if errMsg := setRouteForwardServicesPaused(forward, true); errMsg != "" {
		return dto.Err(errMsg)
	}
	DB.Model(&model.Forward{}).Where("id = ?", forward.ID).Updates(map[string]interface{}{"status": forwardStatusPaused, "updated_time": time.Now().UnixMilli()})
	return dto.Ok("route forward paused")
}

func resumeRouteForward(forward *model.Forward) dto.R {
	route, _, errMsg := LoadRouteWithHops(forward.RouteId)
	if errMsg != "" {
		return dto.Err(errMsg)
	}
	if route.Status != tunnelStatusActive {
	...
	if errMsg := setRouteForwardServicesPaused(forward, false); errMsg != "" {
		return dto.Err(errMsg)
	}
	DB.Model(&model.Forward{}).Where("id = ?", forward.ID).Updates(map[string]interface{}{"status": forwardStatusActive, "updated_time": time.Now().UnixMilli()})
	return dto.Ok("route forward resumed")
}

func diagnoseRouteForward(forward *model.Forward) dto.R {
	_, hops, errMsg := LoadRouteWithHops(forward.RouteId)
	if errMsg != "" {
		return dto.Err(errMsg)
	}
	ports, errMsg := loadForwardRoutePorts(forward.ID)
	if errMsg != "" {
		return dto.Err(errMsg)
	}
	...
	for i := 0; i < len(hops)-1; i++ {
		source := GetNodeById(hops[i].NodeId)
		if source == nil {
			return dto.Err("route node not found")
		}
		next := hops[i+1]
		port := portByHop[next.HopOrder]
		results = append(results, performTcpPingDiagnosis(source, next.NodeIp, port, fmt.Sprintf("route hop %d->%d", i+1, i+2)))
	}
	lastNode := GetNodeById(hops[len(hops)-1].NodeId)
	if lastNode == nil {
		return dto.Err("route terminal node not found")
	}
	for _, addr := range strings.Split(forward.RemoteAddr, ",") {
		targetIp := extractIpFromAddress(addr)
		targetPort := extractPortFromAddress(addr)
		if targetIp == "" || targetPort == -1 {
			return dto.Err("cannot parse target address: " + addr)
		}
		results = append(results, performTcpPingDiagnosis(lastNode, targetIp, targetPort, "route terminal->target"))
	}
...
func createRouteGostServices(forward *model.Forward, route *model.Route, hops []model.RouteHop, ports []model.ForwardRoutePort, limiter *int64, serviceName string) string {
	...
	lastPort := portByHop[lastHop.HopOrder]
	terminalResult := pkg.AddRouteTerminalService(lastHop.NodeId, routeRelayServiceName(serviceName, lastHop.HopOrder), lastPort, forward.RemoteAddr, route.Protocol, forward.Strategy, forward.InterfaceName)
	if !isGostSuccess(terminalResult) {
		return gostResultMessage(terminalResult)
	}

	for i := len(hops) - 2; i >= 1; i-- {
		source := hops[i]
		next := hops[i+1]
		relayPort := portByHop[source.HopOrder]
		chainName := routeChainName(serviceName, i)
		chainRemoteAddr := formatRemoteAddr(next.NodeIp, portByHop[next.HopOrder])
		chainResult := pkg.AddRouteChain(source.NodeId, chainName, chainRemoteAddr, route.Protocol, route.InterfaceName)
		if !isGostSuccess(chainResult) {
			_ = cleanupRouteForwardServices(forward)
			return gostResultMessage(chainResult)
		}
		relayResult := pkg.AddRouteRelayService(source.NodeId, routeRelayServiceName(serviceName, source.HopOrder), relayPort, chainName, route.Protocol, route.InterfaceName)
		if !isGostSuccess(relayResult) {
			_ = cleanupRouteForwardServices(forward)
			return gostResultMessage(relayResult)
		}
	}

	entryChainName := routeChainName(serviceName, 0)
	entryRemoteAddr := formatRemoteAddr(hops[1].NodeIp, portByHop[hops[1].HopOrder])
	entryChainResult := pkg.AddRouteChain(hops[0].NodeId, entryChainName, entryRemoteAddr, route.Protocol, route.InterfaceName)
	if !isGostSuccess(entryChainResult) {
		_ = cleanupRouteForwardServices(forward)
		return gostResultMessage(entryChainResult)
	}
	listenAddr := route.TcpListenAddr
	if forward.ListenIp != "" {
		listenAddr = forward.ListenIp
	}
	entryResult := pkg.AddRouteEntryService(hops[0].NodeId, serviceName, forward.InPort, limiterInt, listenAddr, entryChainName, route.Protocol, route.InterfaceName)
	if !isGostSuccess(entryResult) {
		_ = cleanupRouteForwardServices(forward)
		return gostResultMessage(entryResult)
	}
	return ""
}

func cleanupRouteForwardServices(forward *model.Forward) string {
	if forward.RouteId == 0 {
		return ""
	}
	_, hops, _ := LoadRouteWithHops(forward.RouteId)
	ports, _ := loadForwardRoutePorts(forward.ID)
	serviceName := buildServiceName(forward.ID, forward.UserId, nil)

	if len(hops) > 0 {
		entryResult := pkg.DeleteRouteServices(hops[0].NodeId, []string{serviceName + "_tcp", serviceName + "_udp"})
		if errMsg := gostDeleteError(entryResult); errMsg != "" {
			return errMsg
		}
		for i := 0; i < len(hops)-1; i++ {
			chainResult := pkg.DeleteRouteChain(hops[i].NodeId, routeChainName(serviceName, i))
			if errMsg := gostDeleteError(chainResult); errMsg != "" {
				return errMsg
			}
		}
	}
	for _, port := range ports {
		serviceResult := pkg.DeleteRouteServices(port.NodeId, []string{routeRelayServiceName(serviceName, port.HopOrder)})
		if errMsg := gostDeleteError(serviceResult); errMsg != "" {
			return errMsg
		}
	}
	return ""
...
func setRouteForwardServicesPaused(forward *model.Forward, paused bool) string {
	_, hops, errMsg := LoadRouteWithHops(forward.RouteId)
	if errMsg != "" {
		return errMsg
	}
	...
	entryResult := send(hops[0].NodeId, []string{serviceName + "_tcp", serviceName + "_udp"})
	if !isGostSuccess(entryResult) {
		return gostResultMessage(entryResult)
	}
	for _, port := range ports {
		result := send(port.NodeId, []string{routeRelayServiceName(serviceName, port.HopOrder)})
		if !isGostSuccess(result) {
...
func allocateRouteForwardPorts(route *model.Route, hops []model.RouteHop, specifiedInPort *int, excludeForwardId *int64) (int, []model.ForwardRoutePort, string) {
	if len(hops) < 2 {
		return 0, nil, "route requires at least two nodes"
	}
	reserved := map[int64]map[int]bool{}
	var inPort int
	if specifiedInPort != nil {
		if !isRoutePortAvailable(hops[0].NodeId, *specifiedInPort, excludeForwardId, reserved) {
			return 0, nil, fmt.Sprintf("entry port %d is unavailable", *specifiedInPort)
		}
		inPort = *specifiedInPort
	} else {
		p := allocateRoutePortForNode(hops[0].NodeId, excludeForwardId, reserved)
		if p == nil {
			return 0, nil, "route entry node has no available ports"
		}
		inPort = *p
	}
	reserveRoutePort(reserved, hops[0].NodeId, inPort)

	ports := make([]model.ForwardRoutePort, 0, len(hops)-1)
	for i := 1; i < len(hops); i++ {
		p := allocateRoutePortForNode(hops[i].NodeId, excludeForwardId, reserved)
		if p == nil {
			return 0, nil, fmt.Sprintf("route node %d has no available ports", hops[i].NodeId)
		}
		reserveRoutePort(reserved, hops[i].NodeId, *p)
		ports = append(ports, model.ForwardRoutePort{RouteId: route.ID, HopOrder: hops[i].HopOrder, NodeId: hops[i].NodeId, RelayPort: *p})
	}
	return inPort, ports, ""
}

func allocateRoutePortForNode(nodeId int64, excludeForwardId *int64, reserved map[int64]map[int]bool) *int {
	node := GetNodeById(nodeId)
	if node == nil {
		return nil
	}
	for port := node.PortSta; port <= node.PortEnd; port++ {
		if isRoutePortAvailable(nodeId, port, excludeForwardId, reserved) {
			return &port
		}
	}
	return nil
...
func isRoutePortAvailable(nodeId int64, port int, excludeForwardId *int64, reserved map[int64]map[int]bool) bool {
	node := GetNodeById(nodeId)
	if node == nil || port < node.PortSta || port > node.PortEnd {
		return false
	}
	if reserved[nodeId] != nil && reserved[nodeId][port] {
		return false
	}
	return !getAllUsedPortsOnNode(nodeId, excludeForwardId)[port]
}

func reserveRoutePort(reserved map[int64]map[int]bool, nodeId int64, port int) {
	if reserved[nodeId] == nil {
		reserved[nodeId] = map[int]bool{}
	}
	reserved[nodeId][port] = true
}

func loadForwardRoutePorts(forwardId int64) ([]model.ForwardRoutePort, string) {
	var ports []model.ForwardRoutePort
	if err := DB.Where("forward_id = ?", forwardId).Order("hop_order ASC").Find(&ports).Error; err != nil {
		return nil, "route forward ports not found"
	}
	if len(ports) == 0 {
		return nil, "route forward ports not found"
	}
...
func routePortByHop(ports []model.ForwardRoutePort) map[int]int {
	result := make(map[int]int, len(ports))
	for _, port := range ports {
		result[port.HopOrder] = port.RelayPort
	}
	return result
}

func routeChainName(serviceName string, segmentOrder int) string {
	return fmt.Sprintf("%s_route_%d_chains", serviceName, segmentOrder)
}

func routeRelayServiceName(serviceName string, hopOrder int) string {
	return fmt.Sprintf("%s_route_%d_tls", serviceName, hopOrder)
}

func gostResultMessage(result *dto.GostResponse) string {
	if result == nil {
...
func gostDeleteError(result *dto.GostResponse) string {
	if isGostSuccess(result) {
		return ""
	}
	msg := gostResultMessage(result)
	if strings.Contains(msg, gostNotFoundMsg) {
		return ""
	}
	return msg
}
