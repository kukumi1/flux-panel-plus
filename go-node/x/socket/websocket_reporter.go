...
type SystemInfo struct {
	...
	SingBoxAudit     service.SingBoxAuditStatus `json:"sing_box_audit"`    // Sing-box access audit status
}
...
func (w *WebSocketReporter) collectSystemInfo() SystemInfo {
	...
	// Attach sing-box access-log audit status
	info.SingBoxAudit = service.GetSingBoxAuditStatus()
...
func (w *WebSocketReporter) handleNodeUpdateBinary(data interface{}) error {
	if !atomic.CompareAndSwapInt32(&w.updating, 0, 1) {
		return fmt.Errorf("node update already in progress")
	}
	...
	fmt.Printf("downloading node update: %s\n", downloadURL)

	httpClient := &http.Client{Timeout: 5 * time.Minute}
	resp, err := httpClient.Get(downloadURL)
	if err != nil {
		return fmt.Errorf("download node update: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("download node update: status %d", resp.StatusCode)
	}
	...
	tmpFile, err := os.CreateTemp(filepath.Dir(currentBinary), "."+filepath.Base(currentBinary)+".new-*")
	if err != nil {
	...
	const minBinarySize = 4 * 1024 * 1024
	...
	if written > maxBinarySize {
		return fmt.Errorf("downloaded binary too large: %d bytes", written)
	}
	if resp.ContentLength > 0 && written != resp.ContentLength {
		return fmt.Errorf("downloaded binary size mismatch: got %d bytes, want %d", written, resp.ContentLength)
	}
	if written < minBinarySize {
		return fmt.Errorf("downloaded binary too small: %d bytes", written)
	}

	backupPath := currentBinary + ".bak"
	if err := copyFileForUpdate(currentBinary, backupPath); err != nil {
		fmt.Printf("backup current binary failed: %v\n", err)
	} else {
	...
	if err := os.Rename(tmpPath, currentBinary); err != nil {
		if restoreErr := copyFileForUpdate(backupPath, currentBinary); restoreErr != nil {
			fmt.Printf("restore backup binary failed: %v\n", restoreErr)
		} else {
			_ = os.Chmod(currentBinary, 0755)
			fmt.Println("restored binary from backup")
		}
	...
	if _, err := os.Stat("/.dockerenv"); err == nil {
		...
		if err := copyFileForUpdate(currentBinary, persistPath); err != nil {
			fmt.Printf("persist docker binary failed: %v\n", err)
		} else {
			_ = os.Chmod(persistPath, 0755)
			fmt.Printf("persisted docker binary to %s\n", persistPath)
		}
	}

	fmt.Printf("node update installed successfully (%d bytes); restarting in place\n", written)
	go func() {
		time.Sleep(1 * time.Second)
		if err := reexecCurrentBinary(currentBinary); err != nil {
			fmt.Printf("re-exec after update failed, exiting for external restart: %v\n", err)
			os.Exit(0)
		}
	}()
...
func reexecCurrentBinary(binary string) error {
	if binary == "" {
