...
func (s *defaultService) Serve() error {
	...
	for {
		...
		audit := newAuditConn(s.name, clientAddr, originalConn)
		conn = audit
...
func newAuditConn(serviceName string, clientAddr string, conn net.Conn) *auditConn {
	return &auditConn{Conn: conn, serviceName: serviceName, clientAddr: clientAddr, started: time.Now()}
}

func (c *auditConn) Read(b []byte) (int, error) {
	n, err := c.Conn.Read(b)
	if n > 0 {
		c.inputBytes.Add(int64(n))
	}
	return n, err
}

func (c *auditConn) Write(b []byte) (int, error) {
...
func (c *auditConn) setError(msg string) {
	if msg != "" {
		c.errorText.Store(msg)
	}
}

func (c *auditConn) finish(defaultError string) {
	// Handler recorder hooks produce target-aware audit events. The outer
	// connection wrapper intentionally avoids emitting target-empty duplicates.
}
